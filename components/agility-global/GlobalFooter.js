...
const GlobalFooter = (props) => {
  ...
  return <Footer {...globalFooterProps} />;
};

GlobalFooter.getCustomInitialProps = async function ({
  agility,
  languageCode,
  channelName,
}) {
  ...
  return {
    ...
    column2Links: contentItem.fields.column2Links.map(
      (link) => link.fields.link
    ),
    column3Links: contentItem.fields.column3Links.map(
      (link) => link.fields.link
    ),
    column4Links: contentItem.fields.column4Links.map(
      (link) => link.fields.link
    ),
    footerLinks: contentItem.fields.footerLinks.map((link) => link.fields.link),
  };
};
